...
_SEVERITY_COLOR = {"critical": theme.CRITICAL_HEX, "warning": theme.WARNING_HEX, "info": f"{theme.TEXT_FAINT}"}
...
class PriorityRow(QFrame):
    ...
    def __init__(self, priority: "priorities.Priority", parent=None):
        ...
        color = _SEVERITY_COLOR.get(priority.severity, f"{theme.TEXT_FAINT}")
        self.setStyleSheet(
            f"PriorityRow {{ background: {theme.BG_ELEVATED}; border: 1px solid {theme.BORDER}; border-left: 4px solid {color}; "
            f"border-radius: 6px; }}"
        )
        ...
        site_label.setStyleSheet(f"font-size: 10.5px; font-weight: 700; color: {theme.TEXT_MUTED};")
        ...
        pages_label.setStyleSheet(f"font-size: 11px; color: {theme.TEXT_FAINT};")
...
class TodayView(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.subtitle.setStyleSheet(f"font-size: 12px; color: {theme.TEXT_MUTED};")
    ...
    def refresh(self) -> None:
        ...
        if not priority_list:
            ...
            empty.setStyleSheet(f"color: {theme.TEXT_FAINT}; font-size: 12.5px; font-style: italic;")
