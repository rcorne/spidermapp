...
class LlmVisibilityTab(QWidget):
    ...
    def _build_readiness_section(self, layout: QVBoxLayout) -> None:
        ...
        header.setStyleSheet(f"font-size: 14px; font-weight: 700; color: {theme.TEXT_PRIMARY};")
        ...
        sub.setStyleSheet(f"font-size: 11.5px; color: {theme.TEXT_MUTED};")
        layout.addWidget(sub)

        self.readiness_status = QLabel("Corre un crawl para ver este análisis.")
        self.readiness_status.setStyleSheet(f"font-size: 12px; color: {theme.TEXT_MUTED}; margin-top: 4px;")
        ...
        rule.setStyleSheet(f"color: {theme.BORDER}; margin: 10px 0;")
    ...
    def _on_readiness_report(self, report: ai_visibility.AiVisibilityReport) -> None:
        ...
        bots_header.setStyleSheet(f"font-size: 12px; font-weight: 700; color: {theme.TEXT_SECONDARY}; margin-top: 4px;")
        ...
        if not report.robots_found:
            ...
            note.setStyleSheet(f"font-size: 11.5px; color: {theme.TEXT_FAINT};")
        ...
        for row, bot in enumerate(report.bot_access):
            name = QLabel(bot.bot)
            name.setStyleSheet(f"font-size: 11.5px; font-weight: 600; color: {theme.TEXT_PRIMARY};")
            name.setFixedWidth(120)
            desc = QLabel(bot.description)
            desc.setStyleSheet(f"font-size: 11.5px; color: {theme.TEXT_MUTED};")
        ...
        signals_header.setStyleSheet(f"font-size: 12px; font-weight: 700; color: {theme.TEXT_SECONDARY}; margin-top: 8px;")
        ...
        if report.structured_data_types:
            ...
            types_label.setStyleSheet(f"font-size: 11px; color: {theme.TEXT_FAINT};")
        ...
        if report.wikipedia_found:
            ...
            wiki_label.setStyleSheet(f"font-size: 11.5px; color: {theme.TEXT_FAINT};")
    ...
    def _on_report(self, report) -> None:
        ...
        if report.providers_unconfigured:
            ...
            parts.append(f'<span style=f"color:{theme.TEXT_FAINT};">Sin configurar: {missing}</span>')
